...
/// Reason for a gap in telemetry data.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
pub enum GapReason {
    /// IRSDK connection lost.
    Disconnect,
    /// User reset to pits/track.
    Reset,
    /// Crash or incident.
    Crash,
    /// Entered pit lane.
...
/// Marker indicating a gap in telemetry data.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct GapMarker {
    /// Start time of the gap in milliseconds (session time).
    pub start_time_ms: i64,
    /// End time of the gap in milliseconds (session time).
    pub end_time_ms: i64,
    /// Reason for the gap.
...
mod tests {
    ...
    fn test_add_gap_marker() {
        ...
        assert_eq!(markers[0].start_time_ms, 10000);
        assert_eq!(markers[0].end_time_ms, 10500);
