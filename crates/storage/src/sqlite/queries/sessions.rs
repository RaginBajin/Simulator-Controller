...
pub async fn list_sessions(
    ...
    sql.push_str(&format!(" ORDER BY started_at DESC LIMIT ${} OFFSET ${}", limit_idx, offset_idx));
...
pub async fn update_session(pool: &SqlitePool, id: &str, update: SessionUpdate) -> Result<Session, StorageError> {
...
pub async fn soft_delete_session(pool: &SqlitePool, id: &str) -> Result<DeleteResult, StorageError> {
...
pub async fn restore_session(pool: &SqlitePool, id: &str) -> Result<RestoreResult, StorageError> {
    ...
    let deleted_time = chrono::DateTime::parse_from_rfc3339(deleted_at)
        .or_else(|_| chrono::NaiveDateTime::parse_from_str(deleted_at, "%Y-%m-%dT%H:%M:%S%.3fZ")
            .map(|dt| dt.and_utc().fixed_offset()))
        .map_err(|_| StorageError::InvalidInput(format!("Invalid deleted_at timestamp: {}", deleted_at)))?;
    ...
    let restored_status = session.previous_status.unwrap_or_else(|| "completed".to_string());
    let now = chrono::Utc::now().format("%Y-%m-%dT%H:%M:%S%.3fZ").to_string();
...
pub async fn get_session_detail(pool: &SqlitePool, id: &str) -> Result<SessionDetail, StorageError> {
    let session = get_session(pool, id)
        .await?
        .ok_or_else(|| StorageError::NotFound(id.to_string()))?;
    ...
    let has_telemetry = session.telemetry_path.as_ref().is_some_and(|p| !p.is_empty());
...
pub async fn find_expired_deleted_sessions(pool: &SqlitePool, max_age_days: u32) -> Result<Vec<Session>, StorageError> {
...
pub async fn get_session_stats(pool: &SqlitePool, filters: &FilterOptions) -> Result<SessionStats, StorageError> {
    let mut sql = String::from(
